...
export const CustomerForm = memo(() => {
  ...
  useEffect(() => {
    getCurrentCustomer().then(data => profile.current.value = data[0]);
  }, []);
  useEffect(() => {
    if (feedback !== "") {
  ...
  const handleSaveButtonClick = event => {
    ...
    return putUpdatedCustomerProfile(profile.current.value).then(() => setFeedback("Profile information successfully saved"));
  };
  return <>
        ...
        <form className="profile">
            ...
            <fieldset>
                <div className="form-group">
                    ...
                    <input type="tel" className="form-control" ref={profile} />
                </div>
            </fieldset>
