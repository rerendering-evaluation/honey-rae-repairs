...
export const EmployeeTicket = memo(({
  ticketObject,
  employees,
  getAllTickets
}) => {
  ...
  const claimTicket = memo(() => {
    return <button onClick={() => {
      postClaimedEmployeeTicket(userEmployee, ticketObject).then(getAllTickets);
    }}>Claim</button>;
  });
  const closeTicket = () => {
    const copy = {
    ...
    return putClosedTicket(ticketObject, copy).then(getAllTickets);
  };
  const canClose = memo(() => {
  ...
  return <section className="ticket" key={`ticket--${ticketObject.id}`}>
            ...
            <footer className="ticket__footer">
                {ticketObject.employeeTickets.length ? `Currently being worked on by ${assignedEmployee !== null ? assignedEmployee?.user?.fullName : ""}` : claimTicket()}
                {canClose()}
            </footer>
        </section>;
});